...
SELECT * FROM(
  ...
  app_start_date AS App_Created_Dt,
 ...
 (((((BCC5830 - BCC5838)/0.04)*0.025) + BCC5838) - (BCC5830 - BCX5830)) AS Z,
  ...
  CASE 
  WHEN MTX5839 > 0 AND MTJ0416 > 0 THEN (MTX5839*0.5)
  ...
  (ALL5830  - MTX5839 - AUT5820) AS T_Additional_Bureau_Debt,
...
SELECT 
  *,
  ...
  ROUND((Calc_Income * Tax_Reduction_Factor - T_Housing_Expense- T_Transportation_Expense - T_Additional_Bureau_Debt - ROUND((Amt_To_Refi * 0.0638/12)/ (1 - POW((1 + 0.0638/12), -120)),3)),3)
 -- ELSE ROUND((Calc_Income * Tax_Reduction_Factor - T_Housing_Expense - T_Transportation_Expense- T_Additional_Bureau_Debt - ROUND((Total_Bureau_Reported_SL_Balance * 0.0638/12)/ (1 - POW((1 + 0.0638/12), -120)),3)),3)
  AS T_Pre_Loan_FCF,
  ROUND((Calc_Income * Tax_Reduction_Factor - T_Housing_Expense - T_Transportation_Expense - T_Additional_Bureau_Debt - ROUND((Amt_To_Refi * Offer_Non_Autopay_Rate/12)/ (1 - POW((1 + Offer_Non_Autopay_Rate/12), -(Offer_Term*12))),3)),3)
 -- ELSE ROUND((Calc_Income * Tax_Reduction_Factor - T_Housing_Expense - T_Transportation_Expense - T_Additional_Bureau_Debt - ROUND((Amt_To_Refi * Offer_Non_Autopay_Rate/12)/ (1 - POW((1 + Offer_Non_Autopay_Rate/12), -(Offer_Term*12))),3) - ROUND((Total_Bureau_Reported_SL_Balance * 0.0638/12)/ (1 - POW((1 + 0.0638/12), -120)),3) * (1 - Amt_To_Refi/Total_Bureau_Reported_SL_Balance) ),3)
  AS T_Offer_Post_Loan_FCF, 
  ROUND((Calc_Income * Tax_Reduction_Factor - T_Housing_Expense - T_Transportation_Expense - T_Additional_Bureau_Debt - ROUND((Amt_To_Refi * Selected_Initial_Rate/12)/ (1 - POW((1 + Selected_Initial_Rate/12), -(Selected_Term_Month))),3)),3) 
  -- ELSE ROUND((Calc_Income * Tax_Reduction_Factor - T_Housing_Expense - T_Transportation_Expense - T_Additional_Bureau_Debt - ROUND((Amt_To_Refi * Selected_Initial_Rate/12)/ (1 - POW((1 + Selected_Initial_Rate/12), -(Selected_Term_Month))),3) - ROUND((Total_Bureau_Reported_SL_Balance * 0.0638/12)/ (1 - POW((1 + 0.0638/12), -120)),3) * (1 - Amt_To_Refi/Total_Bureau_Reported_SL_Balance) ),3)
   AS T_Selected_Post_Loan_FCF 
FROM F0),
...
FROM F1
WHERE DATE(App_Created_Dt) < '2020-01-24'
Order by 	App_Created_Dt DESC 
)
SELECT 
  *
FROM F2
WHERE Application_uuid_value IN ('9679f32a-a070-47ee-8ad2-49873fe5b45e', '5a0711ec-b8eb-47c8-8e6d-000dfe929aee','0df49e4d-8d65-4664-9caa-7119a76a35d6','1e12707b-0e4f-4d70-8112-049dd4be84fd')
